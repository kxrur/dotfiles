...
const TagButton = (command) =>
  Button({
    ...
    onClicked: () => {
      chatEntry.buffer.text += `${command} `;
    },
    setup: setupCursorHover,
    label: command,
  });

const CommandButton = (command, displayName = command) =>
  Button({
    className: "sidebar-chat-chip sidebar-chat-chip-action txt txt-small",
...
export const booruTabIcon = Box({
  hpack: "center",
  homogeneous: true,
  children: [MaterialIcon("gallery_thumbnail", "norm")],
});
...
const booruWelcome = Box({
  ...
  child: Box({
    ...
    vpack: "center",
    vertical: true,
    children: [BooruInfo(), BooruSettings()],
  }),
});

const BooruPage = (taglist, serviceName = "Booru") => {
  const PageState = (icon, name) =>
    Box({
  ...
  const PreviewImage = (data, delay = 0) => {
    ...
    const imageBox = Box({
      ...
      attribute: {
        update: (self, data, force = false) => {
          ...
          const widgetWidth = widgetStyleContext.get_property(
            "min-width",
          ...
          if (!force && fileExists(imagePath)) showImage();
            ...
            Utils.timeout(delay, () =>
              Utils.execAsync(["bash", "-c", downloadCommand])
                .then(showImage)
                .catch(print),
            );
        },
      ...
      child: Box({
        ...
        children: [
          ...
          ImageAction({
            ...
            icon: "file_open",
            action: () =>
              execAsync(["xdg-open", `${data.file_url}`]).catch(print),
          }),
          ImageAction({
            name: getString("Go to source"),
            icon: "open_in_new",
            action: () =>
              execAsync(["xdg-open", `${data.source}`]).catch(print),
          }),
          ImageAction({
            name: getString("Save image"),
            icon: "save",
            action: (self) => {
              const currentTags = BooruService.queries
                .at(-1)
                .realTagList.filter((tag) => !tag.includes("rating:"));
              const tagDirectory = currentTags.join("+");
              const fileName = decodeURIComponent(
                data.file_url.substring(data.file_url.lastIndexOf("/") + 1),
              );
          ...
          ImageAction({
            ...
            action: (self) => {
              ...
              execAsync([
                "bash",
                "-c",
                `${saveCommand} && ${setWallpaperCommand}`,
              ])
                .then(() => (self.label = "done"))
                .catch(print);
            },
    ...
    const imageRevealer = Revealer({
      ...
      child: EventBox({
        ...
        onHoverLost: () => {
          imageActions.revealChild = false;
        },
        child: imageOverlay,
      }),
    });
  ...
  const thisPage = Box({
    ...
    attribute: {
      ...
      update: (data, force = false) => {
        // TODO: Use columns. Sort min to max h/w ratio then greedily put em in...
        ...
        // Init cols
        pageImages.children = Array.from({ length: imageColumns }, (_, i) =>
          Box({
            attribute: { height: 0 },
            vertical: true,
          }),
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          // Add image to it
          pageImages.children[minIndex].pack_start(
            PreviewImage(data[i], minIndex),
            false,
            false,
            0,
          );
          pageImages.children[minIndex].attribute.height +=
            1 / data[i].aspect_ratio; // we want height/width
        }
        pageImages.show_all();
    ...
    self
      .hook(
        ...
        (box, id) => {
          ...
          const newPage = BooruPage(
            BooruService.queries[id].taglist,
            BooruService.queries[id].providerName,
          );
          box.add(newPage);
          box.show_all();
      ...
      .hook(
        ...
        (box, id) => {
          ...
          box.attribute.map
            .get(id)
            ?.attribute.update(BooruService.responses[id]);
        },
        "updateResponse",
      ),
});
...
export const booruView = Scrollable({
  ...
  child: Box({
    vertical: true,
    children: [booruWelcome, booruContent],
  }),
  ...
  child: Box({
    ...
    children: [
      Scrollable({
        ...
        child: Box({
          className: "spacing-h-5",
          children: [TagButton("( * )"), TagButton("hololive")],
        }),
      }),
...
export const sendMessage = (text) => {
  ...
  if (text.startsWith("+")) {
    ...
    BooruService.fetch(
      `${lastQuery.realTagList.join(" ")} ${lastQuery.page + 1}`,
    );
